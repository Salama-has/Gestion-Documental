...
async function solicitarRegistro(datosRequeridos) {
  ...
  ajaxrequest.open(
    "POST",
    "https://www.informaticasc.com/curso22_23/salamaDM/DOCS/phpCategorias/consultarRegistro.php", //change
    true
  );
...
function rellenarCampos(registro) {
  ...
  iCategoria.value = registro.caTname;
}
...
function grabarRegistro() {
  ...
  //alert(document.getElementsByName("iCategoria")[0].value);
  console.log(formulario);
  var formData = new FormData(formulario);
  fetch(
    "https://informaticasc.com/curso22_23/salamaDM/DOCS/phpCategorias/grabarRegistro.php", //change
    {
      method: "POST",
      body: formData,
    }
...
function modificarRegistro() {
  ...
  var formData = new FormData(formulario);
  fetch(
    "https://informaticasc.com/curso22_23/salamaDM/DOCS/phpCategorias/modificarRegistro.php",
    {
      method: "POST",
      body: formData,
    }
...
function validarDatos() {
  let validado = true;
  const nombreiCategoria = iCategoria.value.trim();
  ...
  if (validado) {
    if (nombreiCategoria === "") {
      ...
      validado = false;
    } else if (!pattern.test(nombreiCategoria)) {
      mensaje = "El nombre del categoria solo puede contener letras.";
      validado = false;
    }
  }
...
function primerRegistro() {
  //console.log("P");
  let datosRequeridos = "order by categories.id ASC";
  solicitarRegistro(datosRequeridos);
}
...
function ultimoRegistro() {
  //console.log("U");
  let datosRequeridos = "order by categories.id DESC";
  solicitarRegistro(datosRequeridos);
}
...
function siguienteRegistro() {
  ...
  if (iId.value != "") {
    let datosRequeridos =
      "where categories.id>" + iId.value + " order by id ASC";
...
function anteriorRegistro() {
  ...
  if (iId.value != "") {
    let datosRequeridos =
      "where categories.id<" + iId.value + " order by id DESC";
    console.log(datosRequeridos);
    solicitarRegistro(datosRequeridos);
  }
}
...
function borrarRegistro() {
  if (iId.value != "") {
    ...
    ajaxrequest.open(
      "POST",
      "https://www.informaticasc.com/curso22_23/salamaDM/DOCS/phpCategorias/borrarRegistro.php", //change
      true
    );
